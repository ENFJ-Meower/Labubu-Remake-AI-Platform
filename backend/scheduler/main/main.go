...
func (s *Scheduler) Run() {
	...
	for range ticker.C {
		...
		if err != nil || task == "" {
			log.Printf("ERR: Failed to get next task: %v", err)
			continue
		}
		s.limiter.Acquire()
...
func (s *Scheduler) dispatchTask(task string, vft float64) {
	...
	inReq, _ := s.s3Client.GetObjectRequest(&s3.GetObjectInput{Bucket: aws.String(s.bucket), Key: aws.String(inKey)})
	...
	outReq, _ := s.s3Client.PutObjectRequest(&s3.PutObjectInput{Bucket: aws.String(s.bucket), Key: aws.String(outKey), ContentType: aws.String("application/octet-stream")})
	targetURL, err := outReq.Presign(15 * time.Minute)
	if err != nil {
		log.Printf("Failed to presign target URL: %v", err)
	}
